...
?>
        ...
        <div class='user_list'>
        <p>UserId: <?php echo 'tos000'.$row['id']?></p>
        <p>Username: &nbsp;<?php echo $row["username"] ?></p>
        <p>First Name:<?php echo $row["fname"] ?></p>
        <p>Last Name:<?php echo $row["lname"] ?></p>
        <p>Email:<?php echo $row["email"] ?></p>
        
        <input type='submit' name='edit' value='Edit Details' onclick="window.location='edit_user.php'">
        <input type='submit' name='logout' value='Logout' onclick="window.location='logout.php'">
        </div>
        </div>
    </div>
</section>
